#!/bin/sh
# check for value of relative ingress path like this todo-web
# without trailing slash
echo "Ingresss path env var like / or /public/ " $INGRESSPATH
...
if [ -z "$INGRESSPATH" ] || [ -z "$COS_ENDPOINT" ] || [ -z "$COS_URI" ] || [ -z "$S3_ACCESS_KEY" ] || [ -z "$S3_SECRET_KEY" ] || [ -z "$S3_BUCKET" ]; then
